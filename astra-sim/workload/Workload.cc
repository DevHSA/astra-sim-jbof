...
void Workload::issue_mem(shared_ptr<Chakra::ETFeederNode> node) {
    ...
    switch (static_cast<MemoryLocationType>(node->tensor_loc())) {
        ...
        case MemoryLocationType::STORAGE_MEMORY:
            ...
            break;
        case MemoryLocationType::FLASH_MEMORY:
            // flash tier access (LLMServingSim tiered-memory extension)
            sys->flash_mem->issue(node->tensor_size(), wlhd);
            break;
        case MemoryLocationType::JBOF_MEMORY:
            // JBOF/JBOF pooled tier access
            sys->jbof_mem->issue(node->tensor_size(), wlhd);
            break;
        case MemoryLocationType::COLDSTORE_MEMORY:
            // cold-storage tier access
            sys->coldstore_mem->issue(node->tensor_size(), wlhd);
            break;
