...
Sys::Sys(int id,
    ...
    for (auto mem : memory_apis) {
        ...
        if (loc == MemoryLocationType::REMOTE_MEMORY) {
            ...
            this->storage_mem->set_sys(id, this);
        }
        else if (loc == MemoryLocationType::FLASH_MEMORY) {
            this->flash_mem = mem;
            this->flash_mem->set_sys(id, this);
        }
        else if (loc == MemoryLocationType::JBOF_MEMORY) {
            this->jbof_mem = mem;
            this->jbof_mem->set_sys(id, this);
        }
        else if (loc == MemoryLocationType::COLDSTORE_MEMORY) {
            this->coldstore_mem = mem;
            this->coldstore_mem->set_sys(id, this);
        }
        else {
