...
int main(int argc, char* argv[]) {
    ...
    if (is_single) {
      ...
      // deep spill tiers (LLMServingSim tiered-memory extension)
      if (mem_json.contains("flash_mem") && mem_json["flash_mem"].is_object()) {
        json j = mem_json["flash_mem"];
        j["memory-location"] = "FLASH_MEMORY";
        auto path = save_json_to_tmp(j, "flash_mem");
        memory_levels.push_back(std::make_unique<AnalyticalMemory>(path));
        std::remove(path.c_str());
      }

      if (mem_json.contains("jbof_mem") && mem_json["jbof_mem"].is_object()) {
        json j = mem_json["jbof_mem"];
        j["memory-location"] = "JBOF_MEMORY";
        auto path = save_json_to_tmp(j, "jbof_mem");
        memory_levels.push_back(std::make_unique<AnalyticalMemory>(path));
        std::remove(path.c_str());
      }

      if (mem_json.contains("coldstore_mem") && mem_json["coldstore_mem"].is_object()) {
        json j = mem_json["coldstore_mem"];
        j["memory-location"] = "COLDSTORE_MEMORY";
        auto path = save_json_to_tmp(j, "coldstore_mem");
        memory_levels.push_back(std::make_unique<AnalyticalMemory>(path));
        std::remove(path.c_str());
      }
