...
AnalyticalMemory::AnalyticalMemory(
  ...
  if (j.contains("memory-location")) {
    ...
    } else if (mem_loc_type_str.compare("FLASH_MEMORY") == 0) {
      mem_loc_type = MemoryLocationType::FLASH_MEMORY;
    } else if (mem_loc_type_str.compare("JBOF_MEMORY") == 0) {
      mem_loc_type = MemoryLocationType::JBOF_MEMORY;
    } else if (mem_loc_type_str.compare("COLDSTORE_MEMORY") == 0) {
      mem_loc_type = MemoryLocationType::COLDSTORE_MEMORY;
    } else {
  ...
  // Optional network-link term for network-attached deep tiers (JBOF/COLDSTORE).
  // Node-local tiers omit these (default 0 -> no link overhead added).
  link_latency = 0;
  if (j.contains("link-latency")) {
    link_latency = j["link-latency"];
  }

  link_bw = 0;
  if (j.contains("link-bw")) {
    link_bw = j["link-bw"];
  }
...
uint64_t AnalyticalMemory::get_mem_runtime(uint64_t tensor_size) {
  // Device term: access latency (ns) + bytes / bandwidth (GB/s == byte/ns).
  uint64_t runtime = mem_latency;
  if (mem_bw > 0) {
    runtime += static_cast<uint64_t>(static_cast<double>(tensor_size) / mem_bw);
  }
  // Network-link term for network-attached tiers (JBOF/COLDSTORE). Node-local
  // tiers leave link_bw/link_latency at 0, so nothing is added.
  if (link_bw > 0) {
    runtime += link_latency
        + static_cast<uint64_t>(static_cast<double>(tensor_size) / link_bw);
  } else if (link_latency > 0) {
    runtime += link_latency;
  }
  return runtime;
}
