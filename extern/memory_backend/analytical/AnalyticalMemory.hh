...
class AnalyticalMemory : public AstraSim::AstraMemoryAPI, public AstraSim::Callable{
 ...
 private:
  ...
  uint64_t link_latency; // optional network-link latency in nanosec (0 = node-local tier)
  uint64_t link_bw; // optional network-link bandwidth in GB/sec (0 = node-local tier)
